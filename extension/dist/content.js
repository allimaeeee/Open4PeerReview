...
(() => {
  ...
  var tokens = {
    ...
    rating: {
      ...
      exceedsBorder: "#735c00",
      // var(--color-secondary)
      exemplifiesBg: "#041627",
      // var(--color-primary)
      exemplifiesText: "#ffffff",
  ...
  var panelBody;
  var saveStatusEl;
  var completionCountCache = /* @__PURE__ */ new Map();
  ...
  function setGCSaveStatus(status) {
  ...
  async function prefetchSiblingCompletions() {
    ...
    await Promise.all(siblings.map(async (sib) => {
      ...
      const [itemsResp, scoresResp, commentsResp] = await Promise.all([
        send({ type: "GET_RUBRIC_ITEMS", payload: { rubricId: sib.rubric_id } }),
        send({ type: "GET_SCORES", payload: { reviewId: sib.id } }),
        send({ type: "GET_SCORE_COMMENTS", payload: { reviewId: sib.id } })
      ]);
      const items = itemsResp.data ?? [];
      const total = items.length;
      if (total === 0) return;
      const sibScores = /* @__PURE__ */ new Map();
      ...
      const badge = shadow.getElementById(`tab-badge-${sib.id}`);
      if (badge) {
  ...
  function updateCompletion() {
    ...
    const badge = shadow.getElementById(`tab-badge-${selectedReview.id}`);
    if (badge) {
      badge.textContent = `${scored}/${total}`;
      badge.className = `tab-badge ${scored === total ? "tab-badge-complete" : "tab-badge-incomplete"}`;
      badge.style.display = "";
  ...
  function getTorusPageInfo() {
    ...
    const heading = document.querySelector("main h1") ?? document.querySelector('[role="main"] h1') ?? document.querySelector("article h1") ?? document.querySelector("h1");
    let rawTitle = heading?.textContent?.trim() ?? "";
    rawTitle = rawTitle.replace(/^\d+\.\s+/, "");
    const pageName = rawTitle || "OLI Torus";
    const bodyText = document.body.innerText ?? "";
    const isCheckpoint = /SCORED\s+PAGE/i.test(bodyText) || /Assignment\s+requirement/i.test(bodyText);
  ...
  function refreshAnnotationList(rubricItemId) {
    ...
    if (rubricItemId !== UNLINKED_LIST_KEY) {
      ...
      const evLabel = shadow.getElementById(`evidence-label-${rubricItemId}`);
    ...
    container.querySelectorAll(".ann-view-screenshot").forEach((btn) => {
      btn.addEventListener("click", () => {
        const u = btn.dataset.url;
        if (u) openScreenshotModal(u);
      });
    });
    ...
    container.querySelectorAll(".field-clamp").forEach((el) => {
      if (el.scrollHeight <= el.clientHeight + 2) {
        ...
        const btn = container.querySelector(`.ann-expand-btn[data-ann-id="${annId}"][data-field="${field}"]`);
        if (btn) btn.style.display = "none";
      }
    });
    container.querySelectorAll(".ann-expand-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        const annId = btn.dataset.annId;
        const field = btn.dataset.field;
        const fieldEl = container.querySelector(`#field-${annId}-${field}`);
        if (!fieldEl) return;
  ...
  var SVG_PAGE_CONTENT = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>`;
  var SVG_EDIT_ICON = `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>`;
  var SVG_DELETE_ICON = `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/></svg>`;
  var SVG_TAG_ACTION = `<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2"/><rect x="8" y="2" width="8" height="4" rx="1" ry="1"/></svg>`;
  var SVG_TAG_QUICK = `<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>`;
  function annPageLabelHtml(anchor) {
    const pageName = anchor.pageName ?? null;
    if (!pageName) return "";
    const pageType = anchor.pageType ?? null;
  ...
  var SVG_PIN_INLINE = `<svg width="10" height="13" viewBox="0 0 28 36" fill="currentColor" style="flex-shrink:0"><path d="M14 0C6.268 0 0 6.268 0 14C0 24.5 14 36 14 36C14 36 28 24.5 28 14C28 6.268 21.732 0 14 0Z"/></svg>`;
  function clampField(text, annId, field, className) {
  ...
  function renderAnnotationItem(ann) {
    ...
    const screenshotHtml = screenshotUrl ? `<div class="ann-screenshot-row">
    ...
    const gotoBtn = anchorPageUrl ? `<button class="ann-goto" data-id="${ann.id}">\u2197 Go to annotation</button>` : "";
    let primarySectionHtml;
    if (isHotspot) {
      primarySectionHtml = `
      ...
      const quoteContent = quote ? clampField(quote, ann.id, "quote", "ann-quote") : `<div class="ann-no-quote">No annotated text</div>`;
      primarySectionHtml = `
      <div class="ann-sh-row">
  ...
  function editAnnotation(annotationId) {
    const ann = annotations.find((a) => a.id === annotationId);
    if (!ann) return;
    const item = shadow?.querySelector("#ann-" + annotationId);
    if (!item) return;
    const editableEl = item.querySelector("#ann-editable-" + annotationId);
    if (!editableEl) return;
    let currentTag = ann.tag;
    const existingBody = ann.body.trim();
    const criterionOptions = '<option value="">\u2014 No criterion \u2014</option>' + rubricItems.map(
      (ri) => '<option value="' + escHtml(ri.id) + '"' + (ann.rubric_item_id === ri.id ? " selected" : "") + ">" + escHtml(ri.label) + "</option>"
    ).join("");
    const startDisabled = existingBody === "";
    editableEl.innerHTML = '<div class="ann-sh">Comment</div><textarea class="ann-edit-input" rows="3" placeholder="Add a comment...">' + escHtml(existingBody) + '</textarea><div class="ann-sh" style="margin-top:8px;">Tags</div><div class="ann-tag-toggles"><button class="ann-tag-btn' + (currentTag === "action_item" ? " active" : "") + '" data-tag="action_item">Action Item</button><button class="ann-tag-btn' + (currentTag === "quick_fix" ? " active" : "") + '" data-tag="quick_fix">Quick Fix</button></div><div class="ann-sh" style="margin-top:8px;">Link to Criteria</div><select class="ann-criterion-sel">' + criterionOptions + '</select><div class="ann-edit-actions"><span class="ann-edit-hint"' + (startDisabled ? "" : ' style="display:none;"') + '>Add a comment to save</span><button class="ann-edit-cancel">Cancel</button><button class="ann-edit-confirm"' + (startDisabled ? " disabled" : "") + ">Save</button></div>";
    const confirmBtn = editableEl.querySelector(".ann-edit-confirm");
    const editHintEl = editableEl.querySelector(".ann-edit-hint");
    const editTextarea = editableEl.querySelector(".ann-edit-input");
    editTextarea.addEventListener("input", () => {
      const disabled = editTextarea.value.trim() === "";
      confirmBtn.disabled = disabled;
      editHintEl.style.display = disabled ? "" : "none";
    });
    editableEl.querySelectorAll(".ann-tag-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        const tag = btn.dataset.tag;
        currentTag = currentTag === tag ? null : tag;
        editableEl.querySelectorAll(".ann-tag-btn").forEach((b) => b.classList.remove("active"));
        if (currentTag) editableEl.querySelector('.ann-tag-btn[data-tag="' + currentTag + '"]')?.classList.add("active");
      });
    });
  ...
  function openAnnotationPopup(config) {
    const C = tokens.color;
    const F = tokens.font;
    const isEdit = config.mode === "edit";
    const ann = isEdit ? config.annotation : null;
    const existingTag = ann?.tag ?? null;
    const existingBody = (ann?.body ?? "").trim();
    const existingCriterionId = ann?.rubric_item_id ?? null;
    const headerTitle = isEdit ? "EDIT ANNOTATION" : "ADD ANNOTATION";
    const placeholder = config.mode === "create-hotspot" ? "Describe this hotspot..." : "Describe what this evidence shows...";
    const criterionOptions = '<option value="">(save to unlinked annotations)</option>' + rubricItems.map((ri, idx) => {
      const parts = ri.label.split(" \xB7 ");
      const code = parts.length > 1 ? parts[0] : "C" + (idx + 1);
      const raw = parts.length > 1 ? parts.slice(1).join(" \xB7 ") : ri.label;
    ...
    annotationPopup.innerHTML = '<div id="oer-pop-header" style="display:flex;align-items:center;justify-content:space-between;background:' + C.surface + ";padding:10px 14px;border-bottom:1px solid " + sB + ';cursor:grab;user-select:none;"><span style="' + lbl + '">' + headerTitle + '</span><button id="oer-pop-close" style="background:none;border:none;cursor:pointer;color:' + C.textMuted + ';padding:0;display:flex;align-items:center;font-family:inherit;" aria-label="Close"><svg width="14" height="14" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"/></svg></button></div><div style="padding:12px 14px;border-bottom:1px solid ' + sB + ';"><div style="' + lbl + 'margin-bottom:8px;">COMMENT</div><textarea id="oer-pop-body" rows="3" placeholder="' + placeholder + '" style="display:block;width:100%;padding:4px 0;border:none;border-bottom:2px solid ' + C.border + ";background:transparent;font-size:12px;font-family:" + F.body + ";color:" + C.textPrimary + ';resize:none;outline:none;box-shadow:none;box-sizing:border-box;">' + escHtml(existingBody) + '</textarea></div><div style="padding:12px 14px;border-bottom:1px solid ' + sB + ';display:flex;align-items:center;gap:10px;"><span style="' + lbl + 'flex-shrink:0;">TAGS:</span><div style="display:flex;gap:6px;"><button class="oer-tag-btn" data-tag="action_item" style="' + tagBase + "border:1px solid " + (tagActInit ? C.secondary + "66" : C.border) + ";background:" + (tagActInit ? C.secondaryContainer + "99" : "transparent") + ";color:" + (tagActInit ? C.secondary : C.textSecondary) + ';">' + SVG_TAG_ACTION + ' Action Item</button><button class="oer-tag-btn" data-tag="quick_fix" style="' + tagBase + "border:1px solid " + (tagQfxInit ? C.secondary + "66" : C.border) + ";background:" + (tagQfxInit ? C.secondaryContainer + "99" : "transparent") + ";color:" + (tagQfxInit ? C.secondary : C.textSecondary) + ';">' + SVG_TAG_QUICK + ' Quick Fix</button></div></div><div style="padding:12px 14px;border-bottom:1px solid ' + sB + ';"><div style="' + lbl + 'margin-bottom:8px;">LINK TO CRITERIA <span style="font-size:10px;font-weight:400;text-transform:none;letter-spacing:normal;color:' + C.textMuted + ';">(optional)</span></div><div style="position:relative;"><select id="oer-pop-criterion" style="width:100%;padding:4px 20px 4px 0;border:none;border-bottom:2px solid ' + C.border + ";background:transparent;font-size:12px;font-family:" + F.body + ";color:" + C.textPrimary + ';outline:none;cursor:pointer;box-sizing:border-box;appearance:none;-webkit-appearance:none;">' + criterionOptions + '</select><span id="oer-crit-chevron" style="position:absolute;right:0;top:50%;transform:translateY(-50%);pointer-events:none;color:' + C.textMuted + ';display:flex;align-items:center;transition:transform 150ms ease;"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg></span></div></div><div style="padding:10px 14px;display:flex;align-items:center;gap:8px;"><span id="oer-pop-hint" style="font-size:10px;color:' + C.textMuted + ';font-style:italic;flex:1;">Add a comment to save</span><button id="oer-pop-cancel" style="padding:5px 12px;border-radius:0;border:1px solid ' + C.border + ";background:" + C.surfaceCard + ";font-size:11px;font-weight:500;font-family:" + F.body + ";color:" + C.textSecondary + ';cursor:pointer;">Cancel</button><button id="oer-pop-save" style="padding:5px 12px;border-radius:0;border:none;background:' + C.primary + ";color:" + C.onPrimary + ";font-size:11px;font-weight:600;font-family:" + F.body + ';cursor:pointer;">Save Evidence</button></div>';
    let selectedTag = existingTag;
    ...
    const hdrEl = annotationPopup.querySelector("#oer-pop-header");
    if (hdrEl) {
      hdrEl.addEventListener("pointerdown", (e) => {
        if (e.button !== 0 || e.target.closest("#oer-pop-close")) return;
        e.preventDefault();
    ...
    const bodyEl = annotationPopup.querySelector("#oer-pop-body");
    const criterionEl = annotationPopup.querySelector("#oer-pop-criterion");
    const chevronEl = annotationPopup.querySelector("#oer-crit-chevron");
    const saveBtn = annotationPopup.querySelector("#oer-pop-save");
    const hintEl = annotationPopup.querySelector("#oer-pop-hint");
    const updateSaveState = (disabled) => {
    ...
    bodyEl.addEventListener("focus", () => {
      bodyEl.style.borderBottomColor = C.primary;
    });
    bodyEl.addEventListener("blur", () => {
      bodyEl.style.borderBottomColor = C.border;
    });
    criterionEl.addEventListener("focus", () => {
      if (chevronEl) chevronEl.style.transform = "translateY(-50%) rotate(180deg)";
    });
    criterionEl.addEventListener("blur", () => {
      if (chevronEl) chevronEl.style.transform = "translateY(-50%)";
    });
    saveBtn.addEventListener("click", async () => {
      ...
      if (config.mode === "create-text") {
        ...
        const finalAnchor = { ...rawAnchor, pageUrl: window.location.href, pageName: pgName, pageType: pgType };
        const savedAnns = [];
        ...
        const rawAnchor = config.anchor;
        const { pageName: pgName, pageType: pgType } = getTorusPageInfo();
        const savedAnchor = { ...rawAnchor, pageUrl: window.location.href, pageName: pgName, pageType: pgType };
        hideAnnotationPopup();
        exitHotspotMode();
        const savedAnns = [];
    ...
    if (config.mode === "create-text") {
      ...
      if (editAnn.anchor.type === "html-char-offset") {
        const mark = document.querySelector('mark[data-annotation-id="' + editAnn.id + '"]');
        if (mark) {
        ...
        const pin = document.querySelector('.oer-hotspot-marker[data-annotation-id="' + editAnn.id + '"]');
        if (pin) {
  ...
  function showAnnotationTooltipFor(annId, clientX, clientY) {
    ...
    const bodyDisplay = bodyText ? escHtml(bodyText.slice(0, 80)) + (bodyText.length > 80 ? "\u2026" : "") : '<em style="color:' + C.textMuted + ';">No comment written</em>';
    ...
    const tagStyle = "display:inline-flex;align-items:center;gap:4px;font-size:10px;font-weight:600;padding:2px 8px;border-radius:9999px;letter-spacing:0.04em;background:" + C.secondaryContainer + "99;color:" + C.secondary + ";border:1px solid " + C.secondary + "66;";
    ...
    const html = (
      // ── Header ──
      '<div style="padding:12px 16px;border-bottom:1px solid ' + C.border + ";display:flex;align-items:flex-start;gap:8px;background:" + C.surface + ';"><span style="font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:0.1em;flex:1;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;' + (criterionLabel ? "color:" + C.textSecondary + ";" : "color:" + C.textMuted + ";font-style:italic;") + '">' + (criterionLabel ? escHtml(criterionLabel) : "No criterion linked") + '</span><div style="display:flex;gap:4px;flex-shrink:0;"><button class="tt-edit" style="' + iconBtnBase + "color:" + C.textMuted + ';">' + SVG_EDIT_ICON + '</button><button class="tt-delete" style="' + iconBtnBase + "color:" + C.error + ';">' + SVG_DELETE_ICON + '</button></div></div><div style="padding:12px 16px 4px;font-size:12px;color:' + C.textSecondary + ';line-height:1.5;word-break:break-word;">' + bodyDisplay + "</div>" + // ── View full comment (only when truncated) ──
      (bodyText.length > 80 ? '<div style="padding:0 16px 8px;"><button class="tt-view-full" style="background:none;border:none;cursor:pointer;font-size:12px;color:' + C.secondary + ';opacity:0.7;padding:0;display:inline-flex;align-items:center;gap:4px;font-family:inherit;line-height:1.5;transition:opacity 0.1s;">View full comment ' + arrowUpRight + "</button></div>" : "") + // ── Tag pill ──
      (ann.tag ? '<div style="padding:0 16px 12px;"><span style="' + tagStyle + '">' + (ann.tag === "action_item" ? SVG_TAG_ACTION + " Action Item" : SVG_TAG_QUICK + " Quick Fix") + "</span></div>" : !bodyText && !ann.tag ? "" : bodyText.length <= 80 ? '<div style="padding-bottom:12px;"></div>' : "")
    );
    annotationTooltip.innerHTML = html;
    annotationTooltip.style.display = "block";
    const editBtn = annotationTooltip.querySelector(".tt-edit");
    if (editBtn) {
    ...
    const deleteBtn = annotationTooltip.querySelector(".tt-delete");
    if (deleteBtn) {
    ...
    const viewFullBtn = annotationTooltip.querySelector(".tt-view-full");
    if (viewFullBtn) {
      viewFullBtn.addEventListener("mouseenter", () => {
        viewFullBtn.style.opacity = "1";
      });
      viewFullBtn.addEventListener("mouseleave", () => {
        viewFullBtn.style.opacity = "0.7";
      });
  ...
  function enterHotspotMode() {
    ...
    white-space: nowrap;
  `;
    banner.innerHTML = `\u{1F4CD} Click anywhere to place a hotspot &nbsp;<span style="background:rgba(0,0,0,0.1);padding:2px 7px;border-radius:10px;font-size:11px;">Esc to cancel</span>`;
  ...
  function renderContent(state) {
    if (!panelBody) return;
    const panelFt = shadow?.querySelector(".panel-ft");
  ...
  function renderReviewInterface() {
    ...
    const rubricTabsHtml = siblingReviews.length > 0 ? `<div class="rubric-tabs" id="rubric-tabs">${siblingReviews.map((sib) => {
      const cached = completionCountCache.get(sib.id);
      const badge = cached ? `<span class="tab-badge ${cached.scored === cached.total ? "tab-badge-complete" : "tab-badge-incomplete"}" id="tab-badge-${sib.id}">${cached.scored}/${cached.total}</span>` : `<span class="tab-badge tab-badge-incomplete" id="tab-badge-${sib.id}" style="display:none"></span>`;
  ...
  function renderRubricCriteria() {
    ...
    list.innerHTML = rubricItems.map((item, idx) => {
      ...
      const rawName = labelParts.length > 1 ? labelParts.slice(1).join(" \xB7 ") : item.label;
    ...
    list.querySelectorAll(".rating-box").forEach((box) => {
      box.addEventListener("click", (e) => {
        e.stopPropagation();
        const target = e.target;
        if (target.tagName === "TEXTAREA" || target.closest("textarea")) return;
        toggleScore(box.dataset.item, box.dataset.variant);
      });
    });
  ...
  `;
    ...
    <style>
      @font-face {
        font-family: 'Lato';
        font-weight: 400;
        src: url(${Lato_Regular_default}) format('woff2');
      }
      @font-face {
        font-family: 'Lato';
        font-weight: 700;
        src: url(${Lato_Bold_default}) format('woff2');
      }
      @font-face {
        font-family: 'Newsreader';
        font-weight: 200 800;
        src: url(${Newsreader_Variable_default}) format('woff2-variations');
      }
