...
(() => {
  ...
  function editAnnotation(annotationId) {
    ...
    const criterionOptions = [
      ...
      ...rubricItems.map((ri, idx) => {
        const labelParts = ri.label.split(" \xB7 ");
        const code = labelParts.length > 1 ? labelParts[0] : `C${idx + 1}`;
        const rawName = labelParts.length > 1 ? labelParts.slice(1).join(" \xB7 ") : ri.label;
    ...
    const primaryStaticHtml = isHotspot ? `<div class="ann-sh">${SVG_PIN_INLINE} Hotspot</div>` : `<div class="ann-sh">Annotated Text</div>
  ...
  `;
    let selectedTag = ann.tag;
    item.querySelectorAll(".ann-tag-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        const tag = btn.dataset.tag;
        selectedTag = selectedTag === tag ? null : tag;
        item.querySelectorAll(".ann-tag-btn").forEach((b) => {
          b.classList.toggle("active", b.dataset.tag === selectedTag);
        });
      });
    });
    item.querySelector(".ann-view-screenshot")?.addEventListener("click", () => {
      const url = item.querySelector(".ann-view-screenshot")?.dataset.url;
    ...
    item.querySelector(".ann-edit-confirm")?.addEventListener("click", async () => {
      ...
      if (!newBody) return;
      const newCriterionId = item.querySelector(".ann-criterion-sel").value || null;
      setSaveStatus("saving");
      const resp = await send({
  ...
  `;
    ...
    annotationPopup.querySelector("#oer-pop-save")?.addEventListener("click", async () => {
      ...
      const selectedCriterion = annotationPopup.querySelector(".oer-crit-check:checked")?.value ?? null;
